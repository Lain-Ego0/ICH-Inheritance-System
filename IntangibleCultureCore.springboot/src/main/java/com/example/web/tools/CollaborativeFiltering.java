...
public class CollaborativeFiltering {
    // 第一个参数为用户id, HashMap<Integer, Integer> 为用户的关联的物品 比如文创品id+评分 或则文创品id+浏览时间
    // 或则文章id+评分
    ...
    public List<UserCalculateCosineSimilarity> GetUserCalculateCosineSimilarity(Integer userId) {
        ...
        for (Integer otherUserId : otherUserIds) {
            ...
            UserCalculateCosineSimilarity userCalculateCosineSimilarity = new UserCalculateCosineSimilarity(userId,
                    otherUserId, userGoodIds, v);
            // 添加到相似度列表
