                    ...
                    <template v-for="(item, idx) in columnList">
                        ...
                        <el-table-column v-else-if="item.type == 12" v-bind="item" :key="'IMAGES' + idx" :prop="item.key"
                            ...
                            <template slot-scope="{ row }">
                                ...
                                <el-image v-for="(image, idx) in row[`${item.key}`]" :key="idx"
                                    style="width: 50px; height: 40px" :src="image" :preview-src-list="row[`${item.key}`]"
                                    fit="scale-down">
                        ...
                        <el-table-column v-else-if="item.type == 22" v-bind="item" align="center" :key="'ToFixed3' + idx"
                            :prop="item.key" :label="item.title" :width="item.width || 'auto'">
                            <template slot-scope="{ row }">
                                <el-popover trigger="hover" placement="top">
                                    <p>{{ row[`${item.key}`] || ToFixed3 }}</p>
                                    <div slot="reference" class="text-cut">
                                        {{ row[`${item.key}`] | ToFixed3 }}
                                    </div>
                                </el-popover>
                            </template>
                        </el-table-column>
